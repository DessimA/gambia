...
public class MLServiceHttpClient {
  ...
  public MLServiceHttpClient(
      RestTemplate restTemplate, @Value("${app.ml.service-url}") String mlServiceUrl) {
  ...
  public ResultadoAnalise chamarML(SolicitacaoAnalise solicitacao) {
    ...
    var response = restTemplate.postForEntity(mlServiceUrl + "/classificar", request, Map.class);
