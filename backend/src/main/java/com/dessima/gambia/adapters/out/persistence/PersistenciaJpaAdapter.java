...
public class PersistenciaJpaAdapter implements PersistenciaPort {
    ...
    public PersistenciaJpaAdapter(ImovelRepository imovelRepository,
    ...
    public UUID salvarImovel(SolicitacaoAnalise solicitacao) {
        ...
        var entity = new ImovelEntity(id, solicitacao.tipoImovel().name(), solicitacao.quantidadeEquipamentos());
        imovelRepository.save(entity);
        return id;
    }

    @Override
    public UUID salvarAnalise(UUID imovelId, SolicitacaoAnalise solicitacao, ResultadoAnalise resultado) {
        var id = UUID.randomUUID();
        var entity = new AnaliseConsumoEntity(
                id, imovelId,
